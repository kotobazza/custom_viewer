...
void Application::createDiskUsageTable() {
  ...
  tableValues.push_back({text("Device Name"), text("Device Mount Path"),
                         text("Device Filesystem"), text("Total"), text("Free"),
                         text("% Free<float>"), text("% Used<float>"),
                         text("% Free<gauge>"), text("% Used<gauge>")});
  ...
  for (const cv::DiskInfoRecord &record : cv::DiskInfo::getMountedPoints()) {
    ...
    float pFree = record.totalMemory ? static_cast<float>(record.freeMemory) /
                                           record.totalMemory * 100
                                     : 0.0f;
    float pUsed = record.totalMemory
                      ? static_cast<float>(used) / record.totalMemory * 100
                      : 0.f;

    tableValues.push_back(
        {text(record.deviceName) | center, text(record.mountPoint) | center,
         ...
         gaugeRight(pUsed / 100), gaugeRight(pFree / 100)});
  }
  ...
  diskUsageContainer = Renderer(diskUsageContainer, [&] {
    return vbox({mainTable | flex, // EXPANDING
                 text(commandExecutionString) | underlined | bold |
                     bgcolor(Colors::ALT_BACKGROUND_COLOR) | bold |
                     color(Colors::ALT_FOREGROUND_COLOR)});
  });
}
...
void Application::openDirectory(std::string path) {
  ...
  clearPlainText();
  echoCommand("Opened directory: " +
              std::filesystem::path(path).filename().string());
}

void Application::showArchive(std::string path) {
  cv::ArchiveReadingResults output =
      cv::ArchiveWorker::readArchiveEntries(path);
  ...
  std::string content =
      "Reading archive includes...\nArchive [" + path + "]:\n";

  for (auto &ent : output.entries) {
    content += ent + "\n";
  }

  echoCommand("Opened archive: " +
              std::filesystem::path(path).filename().string());

  if (output.entries.size() == 0) {
    echoCommand("Opened empty archive: " +
                std::filesystem::path(path).filename().string());
  }
...
void Application::openPath(std::string path) {
  if (cv::FileManipulation::isDirectory(path)) {
    openDirectory(path);
    tabSelected = std::find(tabValues.begin(), tabValues.end(), "Filesystem") -
                  tabValues.begin();
  }

  else if (cv::FileManipulation::isArchive(path)) {
    showArchive(path);
    tabSelected = std::find(tabValues.begin(), tabValues.end(), "Archive") -
                  tabValues.begin();
    ...
    if (cv::JsonWorker::isValidJson(path) || cv::XmlWorker::isValidXml(path)) {
      ...
      echoCommand("Checking is " + std::string(isJson ? "JSON" : "XML") +
                  " file accessible for Dictionary format. Deserialization " +
                  std::string(serializable.state == cv::ActionState::Done
                                  ? "Done"
                                  : "Undone"));
      ...
      if (serializable.state == cv::ActionState::Done) {
        ...
        echoCommand(std::string("Found Dictionary. Opened ") +
                    (isJson ? "JSON" : "XML") + " in Dictionary tab: " +
                    std::filesystem::path(path).filename().string());
        ...
        openedFilePath = path;
        tabSelected =
            std::find(tabValues.begin(), tabValues.end(), "Dictionary") -
            tabValues.begin();
        return;
      }
    }

    echoCommand("Opened file:" +
                std::filesystem::path(path).filename().string());
    tabSelected = std::find(tabValues.begin(), tabValues.end(), "Filesystem") -
                  tabValues.begin();
    ...
    if (output.content == "")
      echoCommand("Opened empty file:" +
                  std::filesystem::path(path).filename().string());
    isPathFile = true;
    openedFilePath = path;

  } else {
    echoCommand("ERROR: wrong path/type of {Directiry, Archive, Text file}: " +
                path);
  }
}

void Application::saveFileText() {
  if (cv::FileManipulation::isExistingPath(openedFilePath) &&
      cv::FileManipulation::isFile(openedFilePath)) {
...
void Application::moveFileMenuToPath(std::string path) {
  ...
  std::copy(dirContainig.begin(), dirContainig.end(),
...
void Application::zipArchive() {
  ...
  if (cv::ArchiveWorker::createNewZipArchive(newPath, openedFilePath))
    echoCommand("Zipped file: " +
                std::filesystem::path(newPath).filename().string());
  else
...
void Application::createFilesystemContainer() {
  filesystemDownPannel = Container::Horizontal({
      ...
      Button("<-",
             [&] {
               if (usingPath != starterPath)
                 openParentPath(
                     std::filesystem::path(usingPath).parent_path().string());
               else {
      ...
      Button("Create Directory",
             [&] {
               ...
               if (!cv::FileManipulation::isExistingPath(usingPath + "/" +
                                                         pathInputVal)) {
                 cv::FileManipulation::createNewDirectory(usingPath + "/" +
                                                          pathInputVal);
                 moveFileMenuToPath(usingPath);
                 echoCommand("Created new directory: " + usingPath + "/" +
                             pathInputVal);
               } else {
      ...
      Button("Create File",
             [&] {
               ...
               if (!cv::FileManipulation::isExistingPath(usingPath + "/" +
                                                         pathInputVal)) {
                 cv::FileManipulation::createNewFile(usingPath + "/" +
                                                     pathInputVal);
                 moveFileMenuToPath(usingPath);
                 echoCommand("Created new file: " + usingPath + "/" +
                             pathInputVal);
               } else {
                 echoCommand("This entry already exists!");
               }
      ...
      Button("Create archive",
             [&] {
               if (pathInputVal == "") {
                 echoCommand(
                     "Enter some directory name into input string...(/)");
                 return;
               }
               modalSureToSaveArchive = true;
             }),
      Maybe(Button("Save", [&] { modalSureToWrite = true; }), &isPathFile),
      Maybe(Button("Unzip archive", [&] { modalSureToUnzip = true; }),
            &isPathArchive),
  ...
  filesystemUsageContainer = Container::Vertical(
      {Container::Vertical({
           Renderer([&] {
             return text("File") | bold | color(Colors::THEME_COLOR_2) |
                    hcenter;
           }),
           Input(&plainTextString) | borderRounded | flex,
           Renderer([&] {
             return text(commandExecutionString) | underlined |
                    bgcolor(Colors::ALT_BACKGROUND_COLOR) | bold |
                    color(Colors::ALT_FOREGROUND_COLOR);
           }),
       }) | flex,
...
void Application::saveIntoJson() {
  Dictionary t{dictionaryNameInputVal, std::stoi(dictionaryIdInputVal),
               dictionaryContentInputVal};
  if (isPathDictionary) {
    cv::TextWorker::writeIntoFile(openedFilePath,
                                  cv::JsonWorker::serializeDictionary(t));
  } else {
    if (!cv::FileManipulation::createNewFile(openedFilePath))
      return;
    cv::TextWorker::writeIntoFile(openedFilePath,
                                  cv::JsonWorker::serializeDictionary(t));
  }
...
void Application::saveIntoXml() {
  ...
  if (isPathDictionary) {
    cv::TextWorker::writeIntoFile(openedFilePath,
                                  cv::XmlWorker::serializeDictionary(t));
  } else {
    if (!cv::FileManipulation::createNewFile(openedFilePath))
      return;
    cv::TextWorker::writeIntoFile(openedFilePath,
...
void Application::createDictionaryContainer() {
  ...
  dictionaryDownPannel = Container::Horizontal({
      ...
      Button("Save in JSON",
             [&] {
               ...
               if (pathInputVal.length() >= 5 &&
                   pathInputVal.substr(pathInputVal.length() - 5) == ".json ") {
                 openedFilePath = usingPath + "/" + pathInputVal;
      ...
      Button("Save in XML",
             [&] {
               ...
               } else if (pathInputVal.length() >= 4 &&
                          pathInputVal.substr(pathInputVal.length() - 4) ==
                              ".xml ") {
                 openedFilePath = usingPath + "/" + pathInputVal;
  ...
  dictionaryUsageContainer = Container::Vertical({
      Renderer([&] {
        return text("Dictionary") | bold | color(Colors::THEME_COLOR_2) |
               hcenter;
      }),
      Container::Horizontal({Renderer([&] { return text("ID# ") | bold; }),
                             Input(&dictionaryIdInputVal) | underlined |
                                 ...
                                 CatchEvent([&](Event event) {
                                   return event.is_character() &&
                                          dictionaryIdInputVal.size() > 8;
                                 })}),
      Container::Horizontal({Renderer([&] { return text("Name: ") | bold; }),
                             Input(&dictionaryNameInputVal) | underlined}),

      Container::Vertical({
          Renderer([&] {
            return text("Content") | bold | color(Colors::THEME_COLOR_2) |
                   hcenter;
          }),
...
void Application::createArchiveContainer() {
  ...
  archiveContainer = Container::Vertical(
      {Container::Vertical({Renderer([&] {
                              return text("Archive") | bold |
                                     color(Colors::THEME_COLOR_2) | hcenter;
                            }),
                            ...
                            Renderer([&] {
                              return text(commandExecutionString) | underlined |
                                     bold |
                                     bgcolor(Colors::ALT_BACKGROUND_COLOR) |
                                     bold | color(Colors::ALT_FOREGROUND_COLOR);
                            })}) |
           flex,
       archiveDownPannel});
}
...
void Application::createFilesystemMenu() {
  ...
  filesystemMenu = Container::Vertical({
      Renderer([&] {
        return vbox({text("Filesystem menu") | bold | center, separator()});
      }),
      ...
      Menu({&dirEntries, &filesystemMenuSelected}) |
            ...
            if (ret) {
              if (dirEntries[filesystemMenuSelected] == "..") {
                ...
                std::string selectedPath =
                    usingPath + '/' + dirEntries[filesystemMenuSelected];
                openPath(selectedPath);
              }
            }
            return ret;
          ...
          CatchEvent([&](Event event) {
            ...
            if (ret) {
              if (cv::FileManipulation::isDirectory(
                      usingPath + "/" + dirEntries[filesystemMenuSelected])) {
                archiveCreatingPath =
                    usingPath + "/" + dirEntries[filesystemMenuSelected];
                modalSureToSaveArchive = true;
              } else {
                echoCommand("<Ctrl+A> pressed. Select a folder in Filesystem "
                            "Menu before pressing <Ctrl+A>");
              }
            }
            return ret;
          }),
  ...
  mainContainer =
      Container::Horizontal(
          {Container::Vertical({
               ...
               Renderer([&] {
                 return vbox({text("Help") | bold | center, separator(),
                              hbox({
                                  text("Enter") | color(Colors::THEME_COLOR_2) |
                                      border,
                                  text(" Open folder or file") | vcenter,
                              }),
                              hbox({
                                  text("Ctrl") | color(Colors::THEME_COLOR_2) |
                                      border,
                                  text("A") | color(Colors::THEME_COLOR_1) |
                                      border,
                                  text(" Create an archive") | vcenter,
                              }),
                              hbox({
                                  text("Delete") |
                                      color(Colors::THEME_COLOR_1) | border,
           ...
           Container::Vertical({
               ...
               Container::Tab(
                   {diskUsageContainer | flex, filesystemUsageContainer | flex,
                    dictionaryUsageContainer | flex, archiveContainer | flex},
                   &tabSelected),
      ...
      Modal(Container::Vertical(
                {Renderer([&] {
                   return vbox(
                       {text("Undeniable action") | hcenter | bold, separator(),
                ...
                border | bgcolor(Colors::BACKGROUND_COLOR),
            &modalSureToDelete) |
      Modal(
          Container::Vertical(
              {Renderer([&] {
                 return vbox(
                     {text("Can't overwrite a file") | hcenter | bold,
                      separator(),
                      text("This file cannot be overwritten. The file's "
                           "content will be dropped out."),
                      text(openedFilePath) | color(Colors::THEME_COLOR_2)});
               }),
              ...
              {Renderer([&] {
                 return vbox(
                     {text("Sure to serialize into JSON?") | hcenter | bold,
                      separator(), text("The file will be used:"),
                      text(cv::FileManipulation::clearNonRelativePath(
                          openedFilePath)) |
                          color(Colors::THEME_COLOR_2)});
               }),
               Container::Horizontal(
      ...
      Modal(Container::Vertical(
                {Renderer([&] {
                   ...
                   std::string newPath =
                       usingPath + "/" +
                       cv::FileManipulation::transformPathFromArchiveToFolder(
                           openedFilePath);
                 ...
                 Container::Horizontal(
                     {Renderer([&] { return filler(); }) | flex,
                      Button("Anyway",
                             [&] {
                               unzipArchive();
                               echoCommand("Unzipping archive to folder: " +
                                           openedFilePath);
                 ...
                 Container::Horizontal(
                     {Renderer([&] { return filler(); }) | flex,
                      Button("Anyway",
                             [&] {
                               zipArchive();
                               echoCommand("Creating archive from folder" +
                                           openedFilePath);
